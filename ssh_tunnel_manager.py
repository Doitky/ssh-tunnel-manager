...
class SSHTunnelManagerApp:
    ...
    def _build_ui(self):
        ...
        ttk.Button(toolbar, text="+ New Session", command=self._new_session).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Edit", command=self._edit_session).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Duplicate", command=self._duplicate_session).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Delete", command=self._delete_session).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Connect", command=self._connect_session).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="Disconnect", command=self._disconnect_session).pack(side=tk.LEFT, padx=2)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=10)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=5)
        ttk.Button(toolbar, text="Connect All", command=self._connect_all).pack(side=tk.LEFT, padx=2)
    ...
    def _show_context_menu(self, event):
        ...
        menu.add_command(label="Connect", command=lambda: self._connect_session())
        menu.add_command(label="Disconnect", command=lambda: self._disconnect_session())
        menu.add_separator()
        menu.add_command(label="Connect All", command=lambda: self._connect_all())
        menu.add_command(label="Disconnect All", command=lambda: self._disconnect_all())
        menu.add_separator()
        menu.add_command(label="Edit", command=lambda: self._edit_session())
        menu.add_command(label="Duplicate", command=lambda: self._duplicate_session())
        menu.add_separator()
        menu.add_command(label="Delete", command=lambda: self._delete_session())
